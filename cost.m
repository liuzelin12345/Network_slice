function cost=cost(node,deploy,Br,Dr,Vr,drr,R,VNFn,PATH)
    Path=[node(1,1)];
    shareVNFn=zeros([1,7]);
    ...
    c=4*sum(deploy)+Br*(length(Path)-1);
    ...
    for i=1:length(Vr)
        VNFn(1,Vr(1,i))=VNFn(1,Vr(1,i))+deploy(1,i);
    end
    v=0;
    for i=1:length(VNFn)
        if (shareVNFn(1,i)-10*VNFn(1,i)>0)
            v=v+1;
        end
    end
